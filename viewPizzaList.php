...
<body>
    ...
    <div class="container my-3" id="cont">
        ...
        <div class="container my-3" id="cont">
        <div class="col-lg-4 text-center bg-light my-3" style="margin:auto;border-top: 2px groove black;border-bottom: 2px groove black;">
             <select name ="sort_alphabet" class="btn btn-primary">
                <option value="">--select option--</option>
                <option value="l-h" <?php if(isset($_GET['sort_alphabet'])&&$_GET['sort_alphabet']=='Low-High'){echo "selected";}?>>Low-High(Ascending Order)</option>
                <option value="h-l" <?php if(isset($_GET['sort_alphabet'])&&$_GET['sort_alphabet']=='High-Low'){echo "selected";}?>>High-Low(Descending Order)</option>
             </select>
    ...
    <!-- Pizza container starts here -->
        ...
        if(isset($_POST['sort_alphabet'])){
          if($_POST['sort_alphabet']=="l-h"){
            $sort_option="ASC";
          }
          elseif($_POST['sort_alphabet']=="h-l")
          {
            $sort_option="DESC";
          }
          
        }
            $sql = "SELECT * FROM `pizza` WHERE pizzaCategorieId = $id  ORDER BY pizzaPrice $sort_option";
            $result = mysqli_query($conn, $sql);
            $noResult = true;
            while($row = mysqli_fetch_assoc($result)){
                $noResult = false;
